    ...
    */
    ...
    public static void binaryInsertionSort(int[] a) {
        ...
        for(int current = 1; current < a.length; current++) {
            ...
            if(a[current] >= a[high]) { // currently sortable value is the biggest
                continue;
            }
            while(high - low > 1) {
                int mid = (high + low)/ 2;
            ...
            if(a[current] > a[low] && a[current] < a[high]) { // value is between two numbers
                System.arraycopy(a, high, a, high + 1, current - high);
                a[high] = insertValue;
            } else if(a[current] == a[low]) {   // value is equal to lowest
                System.arraycopy(a, low + 1, a, low + 2, current - high);
                a[low + 1] = insertValue;
            } else if(a[current] == a[high]) {  // value is equal to highest
                System.arraycopy(a, high + 1, a, high + 2, current - high - 1);
                a[high + 1] = insertValue;
            } else if(a[current] < a[low]) {    //value is lower than lowest(first index)
